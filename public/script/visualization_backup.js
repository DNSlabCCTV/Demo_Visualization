...
*/
    ...
    socket_local.on("showList",function(data){
      ...
      $(divname).append($ul);
      for(var i=0 ; i< data.length ; i++){  //파일명
        var vid_name = data[i].PATH.split("/");
	var path_name = "";
	for(var j = 3; j < vid_name.length; j++){
	  path_name += "/" + vid_name[j]
	}
        var $li = $('<li id="'+path_name+'" onclick="select_vid(this)"" style="font-size:30px; text-align:center; border: 1px solid black;">'+vid_name[vid_name.length - 1]+'</li>') //파일명, 스토리지
        $($ul).append($li);
      }
    })
