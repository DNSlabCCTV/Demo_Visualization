...
또한, Network Topology를 통해 등록된 기관과 CCTV를 한 눈에 볼 수 있다.
 ...
 건국대 측에서 각 CCTV의 영상을 10분 단위로 나누어 Datalake 및 영상정보DB에 저장한다. 영상정보DB 속성으로는 기관명, 카메라 이름, 날짜, 시간, 저장경로가 있다.
 성균관대 측에서는 10분 단위로 나누어진 영상에 이상현상이 일어나는지 분석하고, 이상현상이 감지되는 영상에 한하여 Datalake 및 AI분석영상에 추가로 저장한다. 이 때, 이상현상 감지된 카메라에 한하여 메세지 창을 띄운다.
 
  Control Room의 Web Server에서는 기관명, 카메라 이름, 날짜, 시간, 저장경로가 기록된 DB에 접근하여 영상들을 볼 수가 있다.
